...
const App = () => {
  ...
  const [mistakes, setMistakes] = useState([]); // Bilinmeyen kelimeler listesi
  ...
  const listen = () => {
    ...
    recognition.onresult = (event) => {
      ...
      // KURAL: Doğru telaffuzda sadece tik çıkar, kart yerinde kalır
  ...
  const handleManualAction = (known) => {
    if (known) {
      ...
      // Bilmiyorsam detaylarıyla listeye ekle
    ...
    setTimeout(() => {
      ...
      if (wordIndex + 1 < levelWords.length) {
        ...
        alert(`Seviye Bitti! Bilmediğin ${mistakes.length} kelimeyi listeye ekledim.`);
...
const s = {
  ...
  mistakeBtn: { marginTop: "10px", background: "#334155", color: "#38bdf8", border: "1px solid #38bdf8", padding: "8px 15px", borderRadius: "20px", fontSize: "11px", fontWeight: "bold" },
  ...
  mistakeItem: { padding: "12px", borderBottom: "1px solid #0f172a", fontSize: "14px", textAlign: "left" },
